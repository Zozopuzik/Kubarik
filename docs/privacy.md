...
Account and game data is stored on **Supabase**, a hosted PostgreSQL database. Row-level security policies restrict each row to its owner. Anonymized public fields (nickname, best score) appear on the in-app leaderboard.
...
Inside the App, tap **Profile → Delete Account**. This immediately removes your auth record, profile, and every game you've played. The action is irreversible.
